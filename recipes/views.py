...
def recipes_list(request):
    if request.method == 'POST':
        ...
        for i in range(len(json_data)-1): 
            ...
            print(i)
            datatemp = {
                'name': settings.TRANSLATOR.translate(text=json_data['results'][0]['title']),
...
def recipe_detail(request, id):
    if request.method == 'GET':
        API_KEY = settings.API_KEY
        id = id
        rqs = requests.get(f'https://api.spoonacular.com/recipes/{id}/information?includeNutrition=false&apiKey={API_KEY}')

        json_data = rqs.json()
        data = {
            'title': settings.TRANSLATOR.translate(text=json_data['title']),
        ...
        for i in range(len(json_data['extendedIngredients'])-1):
            ingrediant_data = {
                'name': settings.TRANSLATOR.translate(text=json_data['extendedIngredients'][i]['originalName']),
                'image': json_data['extendedIngredients'][i]['image'],
            }

            # ingrediant_data = {
            #     'name': json_data['extendedIngredients'][i]['originalName'],
            #     'image': json_data['extendedIngredients'][i]['image'],
            # }
        ...
        if len(json_data['analyzedInstructions']) != 0:
            for i in range(len(json_data['analyzedInstructions'][0]['steps']) -1):
                instructions_data = {
                    'step': settings.TRANSLATOR.translate(text=json_data['analyzedInstructions'][0]['steps'][i]['step'])
                }  
                data['instructions'].append(instructions_data)
            sourceUrl = json_data['sourceUrl']
            data['message'].append('For more details and to know more about the recipe\'s instructions, check the recipe\'s official website ')
...
def ingredients_search(request):
    ...
    if request.method == 'POST':
        ...
        API_KEY = settings.API_KEY
        print(API_KEY)
        rqs = requests.get(f"https://api.spoonacular.com/recipes/findByIngredients?ingredients={fullIngredients}&apiKey={API_KEY}&number=15")
        ...
        for i in range(len(json_data)-1):
            ...
            datatemp = {
                'title': settings.TRANSLATOR.translate(text=json_data[i]['title']),
                'id': json_data[i]['id'],
...
def handle_or_500(request):
    f500=True
    return render(request, '500.html',{'500':f500})